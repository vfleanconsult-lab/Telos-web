...
| Link directo al formulario desde admin | Cada evaluador en "Evaluaciones recientes" (y al crear/agregar uno) tiene ahora un link "Ver formulario" a `/evaluacion/eval/[token]`, para revisar estructura y contenido de la rúbrica sin tener que crear un proceso de evaluación nuevo cada vez. `admin.astro` y `crear.ts` ahora seleccionan/devuelven `token`. |
| Fix reset de contraseña — token se invalidaba solo | `solicitar-reset.ts` generaba un `reset_token` nuevo en cada clic de "¿Olvidaste tu contraseña?", sobreescribiendo el de la fila única `admin_credenciales` y dejando inválido cualquier correo previo sin usar. Ahora reutiliza el token vigente (no vencido) en vez de regenerarlo con cada solicitud. |
| Fix reset de contraseña — CSRF de Astro bloqueaba el botón | Causa real del bug reportado ("No se pudo enviar el correo: error desconocido"): el fetch de "¿Olvidaste tu contraseña?" en `admin.astro` no enviaba `Content-Type`/body. Astro (`security.checkOrigin`, activo por default) trata eso como posible envío de formulario cross-site y responde 403 en texto plano — el `res.json()` del cliente fallaba y caía al mensaje genérico. Se alineó con el patrón `Content-Type: application/json` + body que ya usan todos los demás POST del módulo. También se envolvió `solicitar-reset.ts` en `try/catch` para que cualquier excepción futura devuelva un mensaje real en vez de "error desconocido". |
